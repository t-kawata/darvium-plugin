...
`$_R/scripts/tickets/` 配下。`$_R` は `DARVIUM_PLUGIN_ROOT.md` から読み取る（初回は Step 0 で自動生成）。全スクリプトの詳細は `scripts/tickets/README.md` を参照。
...
### Step 0.5: 過去観察レポートの検索（新規）

新規チケット作成の前に、過去の観察レポートを検索し、現在の実験状態を把握する：

```bash
# tickets/context/*/observation.md を新しい順に最大5件表示
_OBS_REPORTS=$(find "$(pwd)/tickets/context" -name "observation-*.md" -maxdepth 2 2>/dev/null | sort -r | head -5)

if [ -z "$_OBS_REPORTS" ]; then
  echo "過去の観察レポートは見つかりませんでした。初めての実験系列として開始します。"
else
  echo "=== 過去の観察レポート（最大5件）==="
  for f in $_OBS_REPORTS; do
    echo "--- $f ---"
    head -20 "$f"
    echo ""
  done
...
上記の検索結果をユーザーに提示し、以下の観点で対話する：

- 現在の実験がどこまで進んでいるか（どのパラメーターが既に較正されたか）
- どのような現象が既に観察・解釈されたか
- 新規チケットを実験系列のどこに位置づけるべきか

対話で得られた知見は spec の `## Investigation` セクション内に「**参照観察レポート**」として記載すること。
...
2. **証拠の記録**: 調査で得られた証拠（エラー箇所のファイル名・行番号、実際の出力、再現手順など）を spec ファイルの `## Investigation` セクションに書き込む。
   ...
   **加えて、** Step 0.5 で検索した過去観察レポートからの知見を以下の形式で記載する：

   ```markdown
   ### 参照観察レポート

   - tickets/context/0012-foo/observation.md — 信頼伝播減衰係数の較正結果
   - tickets/context/0015-bar/observation.md — 検索ランキングエントロピーの観測結果
   ```
