...
### Step X: spec「計装方法・観測対象」の読み取り（新規）

spec の「計装方法・観測対象」セクションを読み取り、計画に反映する：

```bash
# spec ファイルパスを取得
_SPEC_PATH=$(node "$_R/scripts/tickets/resolve-ticket.js" "$ARGUMENTS" 2>&1 | node -e "
  let d='';process.stdin.on('data',c=>d+=c);process.stdin.on('end',()=>{
    try{const j=JSON.parse(d);console.log(j.specPath)}catch(e){process.exit(1)}
  })
")
grep -A 30 "^## 計装方法・観測対象" "$_SPEC_PATH" 2>/dev/null || echo "[WARN] spec に「計装方法・観測対象」セクションがありません"
```

セクションが存在しない場合：

```
⚠️ 警告: このチケットの spec に「計装方法・観測対象」が定義されていません。
Darvium の観測ベース検証では、計装と観測対象の定義が必須です。
以下のいずれかを選択してください：
1. /make-ticket で spec を更新する（推奨）
2. この plan 内で計装・観測対象を定義する
```

セクションが存在する場合、その内容を後続の plan 策定（計装・観測の実装計画）に反映する。
